...
if (isset($_POST['add'])) {
  ...
  if (find_by_groupName($_POST['group-name']) === false) {
    $session->msg('d', '<b>Xin lỗi!</b> Đã nhập Tên nhóm đã có trong cơ sở dữ liệu!');
    redirect('add_group.php', false);
  } elseif (find_by_groupLevel($_POST['group-level']) === false) {
    $session->msg('d', '<b>Xin lỗi!</b> Đã nhập cấp nhóm đã có trong cơ sở dữ liệu!');
    redirect('add_group.php', false);
  }
  if (empty($errors)) {
    ...
    if ($db->query($query)) {
      ...
      //failed
      $session->msg('d', ' Xin lỗi không thể tạo Nhóm!');
      redirect('add_group.php', false);
    }
  } else {
