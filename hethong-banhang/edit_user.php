...
if (!$e_user) {
  $session->msg("d", "Thiếu id người dùng.");
  redirect('users.php');
}
?>
...
if (isset($_POST['update'])) {
  ...
  if (empty($errors)) {
    ...
    if ($result && $db->affected_rows() === 1) {
      $session->msg('s', "Cập nhật tài khoản");
...
if (isset($_POST['update-pass'])) {
  ...
  if (empty($errors)) {
    ...
    $result = $db->query($sql);
    if ($result && $db->affected_rows() === 1) {
      $session->msg('s', "Mật khẩu người dùng đã được cập nhật ");
      redirect('edit_user.php?id=' . (int)$e_user['id'], false);
    } else {
      ...
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id']; ?>" method="post" class="clearfix">
          <div class="form-group">
            ...
            <input type="password" class="form-control" name="password" placeholder="Nhập người dùng mật khẩu mới">
          </div>
          <div class="form-group clearfix">
