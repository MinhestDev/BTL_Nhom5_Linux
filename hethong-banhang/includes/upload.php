...
class  Media
{
  ...
  public function upload($file)
  {
    ...
    elseif (!$this->file_ext($file['name'])) :
      $this->errors[] = 'File không đúng định dạng';
      return false;
    else :
  ...
  public function process()
  {
    ...
    elseif (empty($this->fileName) || empty($this->fileTempPath)) :
      $this->errors[] = "Vị trí tệp không khả dụng.";
      return false;
    elseif (!is_writable($this->productPath)) :
      $this->errors[] = $this->productPath . " Phải ghi được!.";
      return false;
    elseif (file_exists($this->productPath . "/" . $this->fileName)) :
      $this->errors[] = "File {$this->fileName} đã tồn tại.";
      return false;
    else :
      return true;
    endif;
 ...
 /*--------------------------------------------------------------*/
  public function process_media()
  {
    ...
    if (move_uploaded_file($this->fileTempPath, $this->productPath . '/' . $this->fileName)) {
      ...
      $this->errors[] = "Tệp tải lên không thành công, có thể do quyền trên thư mục tải lên không chính xác.";
      return false;
    }
  }
  /*--------------------------------------------------------------*/
  ...
  public function process_user($id)
  {
    ...
    if (!is_writable($this->userPath)) {
      $this->errors[] = $this->userPath . " Phải ghi được!.";
      return false;
    }
    if (!$id) {
      $this->errors[] = " Thiếu id người dùng.";
      return false;
    }
...
/*--------------------------------------------------------------*/
  public function media_destroy($id, $file_name)
  {
    ...
    if (empty($this->fileName)) {
      $this->errors[] = "Tên tệp ảnh bị thiếu.";
      return false;
    }
    if (!$id) {
      $this->errors[] = "Thiếu id ảnh.";
      return false;
    }
    if (delete_by_id('media', $id)) {
      ...
      $this->error[] = "Xóa ảnh không thành công.";
      return false;
    }
  }
}
